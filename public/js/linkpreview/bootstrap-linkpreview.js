    ...
    LinkPreview.prototype = {
        ...
        initPreviewContainer: function() {
            if (this.getOption("previewContainer")) {
            	...
            	var afterEl = $('<div>');
            	afterEl.insertAfter(this.$element);
                this.$previewContainer = afterEl;
            }
            ...
            if (this.getOption("previewContainerClass")) {
                ...
                this.$previewContainer.addClass("row-fluid");
            }
        },
        ...
        renderPreview: function(url, data, that) {
        	var msgId = that.$element.closest('.mainMsg').attr('id').split('message')[1];
        	var previewUrl = that.$element.attr('href');
            ...
            saveLinkAttachments(msgId,image,title,description,previewUrl);
        },
        ...
        findDescriptionInDom: function($dom) {
        	var description = $dom.find('meta[name="description"]').attr("content") ||
        					  $dom.find('meta[property="og:description"]').attr("content") || 
        ...
        findImageInDom: function($dom) {
            var imageSrc = $dom.find("meta[itemprop=image]").attr("content") ||
                ...
                $dom.find('meta[name="twitter:image"]').attr("content") ||
            ...
            if (imageSrc && !this.validateUrl(imageSrc)) {
                ...
                a.href = this.url;
                if (imageSrc && !imageSrc.match(/^http([s]?):\/\/.*/)) {
...
function saveLinkAttachments(randomId,imageUrl,title,description,url){
